...
def generate_test(n_packets, n_heavy_hitters, n_normal_flows, percentage_n_heavy_hitters=0.9):
    ...
    for _ in range(n_heavy_hitters):#create n different heavy_hitter flows
    ...
    #create heavy hitters:
    for _ in range(n_normal_flows):#create n different normal flows
        flow = get_random_flow()
        #check the flow does not exist
    ...
    for _ in range(n_packets):#make sure the percentage of these 2 flows are 0.9 and 0.1
...
#sudo python3 send_sketch.py --n-pkt 1000 --n-hh 0 --n-sfw 28 --p-hh 0
