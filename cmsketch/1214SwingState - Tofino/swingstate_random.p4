...
control MyIngress(
    ...
    action sketch0_count(){
        meta.index_sketch0 = (myhash0.get({
            hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.srcPort, hdr.tcp.dstPort, hdr.ipv4.protocol}))&0x001f;
    }
   ...
   apply {
        if(hdr.if_control.isValid()){
            ...
            get_if_begin();                     //judge if migration has begun
                                    //if(hdr.ipv4.isValid() && hdr.tcp.isValid() && meta.if_begin==0)  //count sketch
            if(meta.if_begin==0){
                tbl_myhash0.apply();
                tbl_myhash1.apply();
                tbl_myhash2.apply();
            ...
            else{           //begin migration
                if(!hdr.myTunnel.isValid()){  //switch0
                    add_header();
                    hdr.myTunnel.register_id = (random_register_id.get())&0x2;
                    hdr.myTunnel.read_place_id = (random_read_place_id.get())&0x001f;

                    if(hdr.myTunnel.register_id==0)
                        read_sketch0(hdr.myTunnel.read_place_id);
                    else if(hdr.myTunnel.register_id==1)
